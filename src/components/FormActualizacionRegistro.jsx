...
export const FormActualizacionRegistro = ({ permisoInicial, onRegistroExitoso }) => {
    ...
    const [tipoPermiso, setTipoPermiso] = useState(permisoInicial?.tipoPermiso.id || '');
    ...
    const registrarPermiso = async (e) => {
        ...
        try {
            const response = permisoInicial
                ? await axios.put(`${base_url}Permissions`, permisoData)
                : await axios.post(`${base_url}Permissions`, permisoData);
...
export default FormActualizacionRegistro;